...
# 会话与 CSRF Cookie 的 Secure 标记。
# 默认 True：浏览器只在 HTTPS 下回传 Cookie，防止明文 HTTP 上被窃取会话。
# 只有在「纯内网 HTTP、没有证书、且明确接受该风险」的部署里才通过
# DJANGO_COOKIE_SECURE=false 关闭——关闭后 HTTP 下的会话不再有这层保护。
COOKIE_SECURE = env_bool("DJANGO_COOKIE_SECURE", True)  # noqa: F405
