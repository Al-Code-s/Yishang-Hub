...
## 四十、换电脑只做 `git clone`：开发库配置与数据快照入库

**背景。** 要在第二台电脑上继续开发，项目方要求「新机器 `git clone` 即可用，不再手工上传文件」。

**取舍（项目方确认，有意偏离 AGENTS.md 第二条）。** 本仓库是单人私有仓库、只服务一家公司、
开发数据全部为演示数据，因此把 `backend/.env`（开发库口令、`DJANGO_SECRET_KEY`、初始化口令）
取消忽略并入库，并把 `mysqldump` 快照放到 `db/`。**生产要求不变**：生产口令仍只从环境变量注入，
`deploy/`、`compose.yaml` 与文档中不出现真实口令。理由、约束与关闭条件见 `docs/assumptions.md` 第 47 条。

**改动。**

- `.gitignore`：在 `backend/.env` 之后加 `!backend/.env` 例外，并写明原因。
- `.gitattributes`：`db/*.sql -text`，快照按原样（LF）保存，不做换行转换。
- 新增 `db/yishang_platform_2026-09-24.sql`：当前开发库快照（154 张表 / 约 1.27 MB）。
- `docs/backup-restore.md` §8.5：写清「哪些内容已随仓库走」「新机器落地顺序」「如何刷新快照」，
  以及两条警告（开发快照 ≠ 生产备份；快照有时效性，导入前先 `migrate`）。

**为什么数据不靠 `seed_demo_xjys` 重建就够了。** 抽查确认演示标记只覆盖主数据与主要单据
