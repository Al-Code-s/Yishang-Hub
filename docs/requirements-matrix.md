...
| REQ-15-18 | 15 | 按**具体客户公司**造一套覆盖各模块的演示数据 | `core/management/commands/seed_demo_xjys.py` | **已完成**：为「新疆意尚智造科技有限公司」（`XJYS`）造 2026-01-01 起的演示数据，覆盖设备管理、能源与数采、WMS、采购、销售、计划/MRP、MES、QMS、SRM、CRM、EHS 与厂内物流；只写本公司（不动 `YS` 公司），幂等（二次执行「新建 0 条」），`production` 直接拒绝；业务状态一律经 Service 层推进、库存一律经统一库存服务过账 |
