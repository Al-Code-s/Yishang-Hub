...
>
> **最后与代码核对：2026-09-20（对应「客户编码自动生成」增量后的代码）**。
...
| 客户与供应商 | 客户档案与联系人（**新增客户不必手工输入编码**，由系统按编码规则自动生成）；供应商档案、联系人、资质与有效期 | 「客户管理」「供应商管理」 |
...
| 角色 | 超级管理员（`super_admin`，由 `bootstrap_system` 自动绑定，无需手工分配） |
...
| 角色编码 | 名称 | 权限点数 | 说明 |
| --- | --- | --- | --- |
| `super_admin` | 超级管理员 | 173 | 全部权限；账号为 `is_superuser` 时后端下发通配符 `*`，前端按「全部权限」处理 |
...
2. 「系统管理 → 角色权限」→ 选角色 → 勾选权限点；如需限制范围，配置「数据范围」。
   - 权限树的一级分组显示为 **`英文模块（中文名）· 权限点数`**，例如 `core（公共基础）· 12 项`、
     `srm（供应商管理）· 10 项`，照着左侧菜单的业务名找对应分组即可；
     「系统管理 → 权限与菜单」的权限点选项卡使用同一套分组名称。
...
- **登录后落到哪一页**：登录成功后自动进入**你菜单里的第一个页面**（通常是「工作台」）；
  直接访问站点根路径 `/` 也会跳到这里。因此不会再出现「顶栏写着工作台、内容区却是空白」的情况；
  如果你没有工作台的查看权限，会落到你自己第一个可访问的页面。
- **顶部工具栏**：折叠侧边栏、面包屑、**使用说明**、通知/待办入口、个人中心
  （个人中心下拉里还有「使用说明」与「接口文档」）。面包屑显示当前页面在菜单里的层级，
  例如「基础资料 / 物料档案」，而不是固定显示「工作台」。
...
| 编码自动生成 | 部分主数据的编码由后端按**编码规则**生成，新增表单里**不出现**该字段（客户档案即如此：默认 `CUS{YYYY}{SEQ:4}`，按年重置，形如 `CUS20260001`）。规则可在「系统管理 → 编码规则」调整格式与重置周期，并支持**编号预演**（预演不消耗流水号）；改规则即改后续新编号，历史编码不变 |
...
| 数值显示 | 列表、详情、看板里的数量 / 单价 / 金额 / 工时**统一显示 2 位小数**（四舍五入 + 千分位），如 `1,234.57`。这是**显示口径**，数据库与接口仍是 6 位精度（如 `1234.567800`），不会因为显示而改写数据。唯一例外：某个非零值四舍五入后正好是 0（如 BOM 用量 `0.004`），会保留必要精度显示为 `0.004`，避免把「有」显示成「没有」。编辑表单回填时只去掉末尾无意义的 0（`12.000000` → `12`），**不做四舍五入**，所以「打开编辑再保存」不会改变你录入的数值。整数计数列（如日产能、待办数量）不显示小数 |
...
| 审计与登录日志 | 操作日志（只读，不可修改）与登录尝试记录；「对象类型」与「变更内容」都显示中文（如「角色」「归属公司 空 → 2」），不使用内部英文标识 |
| 我的通知 | 审批、告警等通知 |
| 实施进度 | 当前实施到哪一步、已登记权限/菜单计数、未完成事项 |

> 表格里的「业务对象」「对象编号」「事件编号」都是排查问题用的业务标识：「业务对象」显示中文名称（如「销售订单」），不会出现 `sales.SalesOrder` 这类内部写法；「事件编号」是系统内部的事件编码，管理员定位失败原因时需要用。

> **本地开发时事件可能长期停在「待处理」**：后台任务未启动时不会消费事件，
> 这是环境限制（见 §九），不代表业务失败。
...
| `python -m pytest tests -q --reuse-db` | `313 passed in 104.03s`（含 `tests/test_docs_sync.py` 7 条文档同步、`tests/test_enum_labels.py` 4 条枚举标签、管理员角色绑定 1 条） |
...
| 页面看不到「新增 / 编辑 / 删除」按钮 | 当前账号缺少对应操作权限点，由管理员在「系统管理 → 角色管理」授予（见 §4.5）。**超级管理员例外**：后端下发通配符 `*`（已按「全部权限」处理），若仍看不到请强制刷新页面（`Ctrl+F5`）或确认前端已更新到最新版本 |
